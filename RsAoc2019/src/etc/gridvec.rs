...
pub struct GridVec<T>{pub vec: Vec<T>, y_min: i64, x_min : i64, y_max: i64, x_max : i64}
    ...
    fn row_slice (&self, y: i64 ) -> &[T] 
{
    ...
    pub fn neighbors4(&self, y: i64, x: i64) -> Vec<GridPos>{
        [(y-1, x), (y, x+1), (y+1, x), (y, x-1)].into_iter().filter(|nei| self.in_range(nei)).collect() 
    ...
    pub fn remap<S>(&self, f: impl Fn(&T) -> S) -> GridVec<S> 
    {
        ...
        GridVec {vec: remapped_vec, y_min: self.y_min, x_min: self.x_min, y_max: self.y_max, x_max: self.x_max}
    }
...
impl<T: Clone> GridVec<T> 
{
    pub fn new(init_val: T, y_min : i64, x_min : i64, y_max : i64, x_max : i64) -> Self {
        ...
        Self{vec: vec![init_val;(width*height) as usize],y_min, x_min, y_max, x_max }
    }
}
