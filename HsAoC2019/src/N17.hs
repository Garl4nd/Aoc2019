...
import Debugging (traceWInfo, traceWInfo2)
import IntCode
import Useful
...
findPath grid initPos = go initPos N
 where
  ...
  go pos facing =
    let
     ...
     in
      case find (not . null . snd) $ straightWalk <$> possibleTurns of
        Nothing -> []
...
intersectionScore grid = sum . map (uncurry (*)) $ [(y - 1, x - 1) | pos@(y, x) <- A.indices grid, all (\pos' -> inBounds pos' && grid ! pos' == '#') (pos : neighbors4 pos)]
 where
  inBounds = A.inRange (A.bounds grid)
