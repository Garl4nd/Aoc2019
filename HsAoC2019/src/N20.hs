...
makeGraph tileArray = A.array (A.bounds tileArray) $ makeEdges <$> A.assocs tileArray
 where
  makeEdges (pos, val)
...
makeGraph2 tileArray = LazyGraph{edgeFunc = getEdges, bounds = ((minBounds, 0), (maxBounds, innerPortalNum))}
 where
  ...
  getEdges (pos, level)
    ...
    | Portal portalName <- tileArray ! pos =
        [((nei, level), Dist 1) | nei <- freeNeighbors pos]
...
shortestPath tileAr =
  let
   ...
   in
    distanceMap (runDijkstra graph startPos [endPos]) ! endPos

shortestPath2 :: TileArray -> Distance
shortestPath2 tileAr =
  let
    graph = makeGraph2 tileAr
    [startPos] = [pos | (pos, Portal "AA") <- A.assocs tileAr]
    [endPos] = [pos | (pos, Portal "ZZ") <- A.assocs tileAr]
   in
    (distanceMap $ runDijkstra @(A.Array AugPos Distance) graph (startPos, 0 :: Int) [(endPos, 0)]) ! (endPos, 0)
